...
public class AuthService {
	...
	public AuthResponse login(LoginRequest loginRequest) {
		try {
			...
			return new AuthResponse(user.getId(), user.getFullName(), user.getEmail(), user.getApiCallCount(), "Login successful", accessToken,
					refreshToken.getToken(), accessTokenExpiration);
		} catch (IllegalArgumentException e) {
			throw e;
		} catch (Exception e) {
	...
	public TokenRefreshResponse refreshToken(TokenRefreshRequest request) {
		try {
			...
			return refreshTokenService.findByToken(refreshTokenStr).map(refreshTokenService::verifyExpiration)
					.map(oldToken -> {
						...
						return new TokenRefreshResponse(newAccessToken, newRefreshToken.getToken(),
								jwtService.getAccessTokenExpirationTime(), "Token refreshed successfully");
